...
| `EMAIL_NOT_VERIFIED` | 403 | false | 이메일 확인을 마치지 않은 계정의 로그인. 재발송 경로를 함께 안내한다 | 01 §5.3, ADR-0023 |
| `FORBIDDEN_SCOPE` | 403 | false | 역할·조직·장소 범위 밖 | CORE-AC-11 |
| `NOT_FOUND` | 404 | false | **존재 여부를 감춰야 할 때도 이 코드를 쓴다** | CORE-AC-10 |

`UNAUTHENTICATED`와 `EMAIL_NOT_VERIFIED`를 가르는 이유는 **사용자가 할 일이 다르기 때문이다.** 앞은 비밀번호를 다시 입력하거나 재로그인할 일이고, 뒤는 메일함을 열어야 할 일이다. 하나로 뭉치면 클라이언트가 안내 문구를 고를 수 없다.

로그인 실패 자체는 계정 존재 여부를 노출하지 않는다(`01 §5.3` — "로그인 실패는 계정 존재 여부를 과도하게 노출하지 않는 메시지"). 그래서 **비밀번호가 틀린 경우와 없는 계정은 같은 응답**이다. `EMAIL_NOT_VERIFIED`는 비밀번호가 맞은 뒤에만 나오므로 이 원칙과 충돌하지 않는다.

---

### 인증·계정

ADR-0023이 인증을 범위에 넣으면서 는 코드다. 위 `EMAIL_NOT_VERIFIED`도 같은 묶음이지만 403이므로 권한 절에 둔다.

| 코드 | HTTP | retryable | 설명 | 근거 |
|---|---|---|---|---|
| `EMAIL_ALREADY_REGISTERED` | 409 | false | 가입하려는 주소가 이미 쓰이고 있다 | 01 §5.3 B2C-AUTH-002 |
| `ACCOUNT_SUSPENDED` | 423 | false | 이용 정지 상태의 계정이다 | 01 §2 |
| `VERIFICATION_TOKEN_EXPIRED` | 410 | false | 이메일 확인·비밀번호 재설정 토큰이 만료됐거나 이미 쓰였다. **새 토큰을 받아야 한다** | ADR-0023 결정 6 |
| `TOO_MANY_REQUESTS` | 429 | **true** | 토큰 재발송 요청이 잦다. `details.retryAfterSeconds` 포함 | ADR-0023 결정 6 |

**`EMAIL_ALREADY_REGISTERED`는 계정의 존재를 알려 준다.** 로그인 실패를 뭉개는 원칙(01 §5.3)과 방향이 반대인데, 의도한 예외다. 감추려면 가입이 성공한 척해야 하고, 그러면 사용자는 오지 않을 메일을 기다린다. 이 코드를 받은 클라이언트는 로그인이나 비밀번호 재설정으로 안내한다.

반면 **재발송과 비밀번호 재설정 요청은 감춘다.** 두 경로 모두 계정이 없어도 `202`다. 거기서는 감춰도 사용자가 잃는 것이 없다 — 어차피 메일을 기다리는 흐름이고, 가르면 그 경로가 가입 여부를 묻는 조회 API가 된다.

**`VERIFICATION_TOKEN_EXPIRED`가 404가 아닌 이유.** 없는 토큰은 `NOT_FOUND`이고, 이것은 "있었지만 지났다"이다. 410을 받으면 클라이언트가 재발송 버튼을 띄울 수 있지만 404에는 띄울 근거가 없다. `IDEMPOTENCY_KEY_EXPIRED`와 같은 꼴이다.

**`ACCOUNT_SUSPENDED`에 423을 쓴 이유.** 403은 "이 리소스에 권한이 없다"이고 여기는 자격 증명이 맞는데도 계정 자체가 잠긴 경우다. 423 Locked가 그 뜻에 가장 가깝다. 다만 이 코드는 WebDAV(RFC 4918)에서 왔고 일반 HTTP에서 흔치 않아, 403으로 낮추는 선택도 가능했다. 가른 이유는 클라이언트가 "권한 없음"과 "정지된 계정"에 서로 다른 안내를 해야 하기 때문이다.
